...
public interface ISpellModifier extends ISpellPart {
	 ...
	 */
	float getModifier(SpellModifiers type, EntityLivingBase caster, Entity target, World world, byte[] metadata);
	 ...
	 */
	float getManaCostMultiplier(ItemStack spellStack, int stage, int quantity);
	 ...
	 */
	byte[] getModifierMetadata(ItemStack[] matchedRecipe);
}
