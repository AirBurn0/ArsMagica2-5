					...
					for(int i = 0; i < 4; ++i) {
						...
						if(effect != null) {
							...
							if(AMCore.config.LowGFX()) {
								effect.AddParticleController(new ParticleOrbitPoint(effect, posX, posY, posZ, 2, false).setIgnoreYCoordinate(true)
																													   .SetOrbitSpeed(0.05f)
																													   .SetTargetDistance(dist)
																													   .setRotateDirection(true));
							}
						}
					}
				}
	...
	private void blizzardUpdate() {
		...
		if(worldObj.isRemote) {
			...
			if(rand.nextInt(10) < 2) {
				...
				int pZ = (int)(posZ - radius + rand.nextInt((int)Math.ceil(radius) * 2));
				if(worldObj.isAirBlock(pX, pY, pZ) && !worldObj.isAirBlock(pX, pY - 1, pZ) && worldObj.getBlock(pX, pY, pZ)
																									  .isOpaqueCube()) {
