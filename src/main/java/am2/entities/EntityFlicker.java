...
public class EntityFlicker extends EntityAmbientCreature {
	...
	public void onUpdate() {
		...
		if(!worldObj.isRemote) {
			for(Object o: worldObj.playerEntities) {
				if(me.distanceSqTo(new AMVector3((EntityPlayer)o)) < 25) {
					...
					if(!ArmorHelper.isInfusionPreset(chestArmor, GenericImbuement.flickerLure)) {
						playerClose = true;
					}
					break;
				}
		...
		if(this.ticksExisted > 100 && playerClose && this.dataWatcher.getWatchableObjectByte(WATCHER_AMBIENTFLICK) == (byte)0) {
			if(this.getActivePotionEffects().size() == 0 || (this.getActivePotionEffects()
																 .size() == 1 && worldObj.rand.nextDouble() < 0.1f
			)) {
				this.dataWatcher.updateObject(WATCHER_AMBIENTFLICK, (byte)1);
			}
		}
		...
		if(this.worldObj.isRemote) {
			for(int i = 0; i < 10 * AMCore.config.getGFXLevel(); ++i) {
				...
				if(particle != null) {
					...
					particle.AddParticleController(new ParticleFadeOut(particle, 1, false).setFadeSpeed((float)(worldObj.rand.nextDouble() * 0.1 + 0.05))
	...
	public boolean getCanSpawnHere() {
		...
		if(biome != null) {
			...
			if(this.worldObj.checkNoEntityCollision(this.boundingBox) && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox)
																					  .isEmpty()) {
				AMCore.proxy.incrementFlickerCount();
				return true;
			}
