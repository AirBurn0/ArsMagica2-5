...
public class EntityAICookNearbyFood extends EntityAIBase {
	...
	private final EntityFireElemental entityHost;
	...
	public boolean shouldExecute() {
		return ExtendedProperties.For(entityHost).getInanimateTarget() != null && !ExtendedProperties.For(entityHost)
																									 .getInanimateTarget().isDead;
	}

	@Override
	public boolean continueExecuting() {
		return ExtendedProperties.For(entityHost)
								 .getInanimateTarget() != null && entityHost.getAttackTarget() != null && !ExtendedProperties.For(entityHost)
																															 .getInanimateTarget().isDead;
	}
	...
	public void updateTask() {
		...
		if(distance > 9D) {
			this.entityHost.getNavigator()
						   .tryMoveToXYZ(inanimate.posX, inanimate.posY, inanimate.posZ, this.entityHost.getAIMoveSpeed());
		}
		else {
			...
			if(timeSpentCooking < 30) {
				...
				ItemStack smelted = FurnaceRecipes.smelting()
												  .getSmeltingResult(((EntityItem)inanimate).getEntityItem());
