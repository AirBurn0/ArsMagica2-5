...
public class EntityAISummonFollowOwner extends EntityAIBase {
	private final EntityCreature theSummon;
	...
	public void updateTask() {
		this.theSummon.getLookHelper()
					  .setLookPositionWithEntity(this.theOwner, 10.0F, (float)this.theSummon.getVerticalFaceSpeed());
					...
					if(this.theSummon.getDistanceSqToEntity(this.theOwner) >= 144.0D) {
						...
						for(int l = 0; l <= 4; ++l) {
							for(int i1 = 0; i1 <= 4; ++i1) {
								if((l < 1 || i1 < 1 || l > 3 || i1 > 3) && World.doesBlockHaveSolidTopSurface(this.theWorld, i + l, k - 1, j + i1) && !this.theWorld.getBlock(i + l, k, j + i1)
																																									.isNormalCube() && !this.theWorld.getBlock(i + l, k + 1, j + i1)
																																																	 .isNormalCube()) {
									this.theSummon.setLocationAndAngles((float)(i + l) + 0.5F, k, (float)(j + i1) + 0.5F, this.theSummon.rotationYaw, this.theSummon.rotationPitch);
									this.petPathfinder.clearPathEntity();
									return;
								}
							}
