...
public class EntityLightMage extends EntityCreature {
	...
	int hp;
	private static final ItemStack diminishedHeldItem = new ItemStack(ItemsCommonProxy.bookAffinity, 1, 2);
	private static final ItemStack normalHeldItem = new ItemStack(ItemsCommonProxy.bookAffinity, 1, 7);
	private static final ItemStack augmentedHeldItem = new ItemStack(ItemsCommonProxy.bookAffinity, 1, 5);
	...
	protected boolean interact(EntityPlayer player) {
		...
		if(SkillData.For(player)
					.isEntryKnown(SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("MageBandI")))) {
			if(EntityUtilities.isSummon(this)) {
				...
				if(ExtendedProperties.For(player).getCanHaveMoreSummons()) {
					if(ExtendedProperties.For(player).getMagicLevel() - 5 >= ExtendedProperties.For(this)
																							   .getMagicLevel()) {
