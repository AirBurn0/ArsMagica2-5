...
public class AMEventHandler {
	...
	public void onLivingDrops(LivingDropsEvent event) {
		...
		if(!event.entityLiving.worldObj.isRemote && event.entityLiving instanceof EntityPig && event.entityLiving.getRNG()
																												 .nextDouble() < 0.3f) {
	...
	public void onEntityFall(LivingFallEvent event) {
		...
		//slowfall buff
		if(ent.isPotionActive(BuffList.slowfall.id) || ent.isPotionActive(BuffList.shrink.id) || (ent instanceof EntityPlayer && AffinityData.For(ent)
																																			 .getAffinityDepth(Affinity.NATURE) == 1.0f
		)) {
			event.setCanceled(true);
			ent.fallDistance = 0;
			return;
	...
	public void onEntityLiving(LivingUpdateEvent event) {
		...
		if(event.entityLiving.isPotionActive(BuffList.slowfall)
				...
				|| (ent instanceof EntityPlayer && AffinityData.For(ent)
															   .getAffinityDepth(Affinity.NATURE) == 1.0f && !ent.isSneaking()
		)) {
	...
	public void onEntityHurt(LivingHurtEvent event) {
		...
		if(event.entityLiving.isPotionActive(BuffList.manaShield)) {
			float manaToTake = Math.min(ExtendedProperties.For(event.entityLiving)
														  .getCurrentMana(), event.ammount * 250f);
	...
	public void onManaCost(ManaCostEvent event) {
		if(event.caster.getHeldItem() != null && event.caster.getHeldItem()
															 .getItem() == ItemsCommonProxy.arcaneSpellbook) {
	...
	public void onPlayerPickupItem(EntityItemPickupEvent event) {
		...
		if(!event.entityPlayer.worldObj.isRemote && ExtendedProperties.For(event.entityPlayer)
																	  .getMagicLevel() <= 0 && event.item.getEntityItem()
		...
		if(event.item.getEntityItem().getItem() == ItemsCommonProxy.spell) {
			...
			if(event.entityPlayer.worldObj.isRemote &&
					...
					(item instanceof ItemBlock && AMCore.proxy.blocks.getArsMagicaBlocks()
																	 .contains(((ItemBlock)item).field_150939_a)
					)) {
				AMNetHandler.INSTANCE.sendCompendiumUnlockPacket((EntityPlayerMP)event.entityPlayer, item.getUnlocalizedName()
																										 .replace("item.", "")
																										 .replace("arsmagica2:", "")
																										 .replace("tile.", "") + ((meta > -1) ? "@" + meta : ""), false);
			}
		}
	}
}
