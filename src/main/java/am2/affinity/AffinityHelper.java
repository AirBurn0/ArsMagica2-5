...
public class AffinityHelper {
	...
	public void onEntityLivingBase(LivingUpdateEvent event) {
		...
		if(enderDepth >= 0.75f && affinityData.hasActivatedNightVision()) {
			if(!ent.worldObj.isRemote && (!ent.isPotionActive(Potion.nightVision.id) || ent.getActivePotionEffect(Potion.nightVision)
																						   .getDuration() <= 220
			)) {
		...
		if(ent.isInWater()) {
			...
			if(waterDepth > 0.5f) {
				if(!ent.isPotionActive(BuffList.swiftSwim.id) || ent.getActivePotionEffect(BuffList.swiftSwim)
																	.getDuration() < 10) {
			...
			if((ent instanceof EntityPlayer && ((EntityPlayer)ent).inventory.armorInventory[1] != null && ((EntityPlayer)ent).inventory.armorInventory[1].getItem() == ItemsCommonProxy.waterGuardianOrbs)) {
				...
				if(!ent.worldObj.isRemote && (!ent.isPotionActive(BuffList.waterBreathing) || ent.getActivePotionEffect(BuffList.waterBreathing)
																								 .getDuration() <= 200
				)) {
		...
		if(ent.worldObj.isRaining() && !ent.worldObj.isRemote && ent.worldObj.getBiomeGenForCoords((int)Math.floor(ent.posX), (int)Math.floor(ent.posZ))
	...
	private void applyFulmintion(EntityPlayer ent, float lightningDepth) {
		...
		if(lightningDepth > 0.25f && ent.isWet()) {
			...
			if(ent.worldObj.isRemote) {
				AMCore.proxy.particleManager.BoltFromEntityToPoint(ent.worldObj, ent, ent.posX - 2 + ent.getRNG()
																										.nextDouble() * 4, ent.posY + ent.getEyeHeight() - 2 + ent.getRNG()
																																								  .nextDouble() * 4, ent.posZ - 2 + ent.getRNG()
																																																	   .nextDouble() * 4);
			}
			else {
	...
	private void applyReverseWaterMovement(EntityLivingBase entity) {
		...
		AxisAlignedBB par1AxisAlignedBB = entity.boundingBox.expand(0.0D, -0.4000000059604645D, 0.0D)
	...
	public void onEntityHurt(LivingHurtEvent event) {
		...
		if(event.source.getSourceOfDamage() instanceof EntityPlayer) {
			float attackerFireDepth = AffinityData.For((EntityLivingBase)event.source.getSourceOfDamage())
												  .getAffinityDepth(Affinity.FIRE);
			if(attackerFireDepth > 0.8f && ((EntityPlayer)event.source.getSourceOfDamage()).getCurrentEquippedItem() == null) {
				ent.setFire(4);
				event.ammount += 3;
			}
			float attackerLightningDepth = AffinityData.For((EntityLivingBase)event.source.getSourceOfDamage())
													   .getAffinityDepth(Affinity.LIGHTNING);
		...
		if(event.source.getSourceOfDamage() instanceof EntityLivingBase) {
			float natureDepth = affinityData.getAffinityDepth(Affinity.NATURE);
			if(natureDepth == 1.0f) {
				event.source.getSourceOfDamage().attackEntityFrom(DamageSource.cactus, 3);
			}
			else if(natureDepth >= 0.75f) {
				event.source.getSourceOfDamage().attackEntityFrom(DamageSource.cactus, 2);
			}
			else if(natureDepth >= 0.5f) {
				event.source.getSourceOfDamage().attackEntityFrom(DamageSource.cactus, 1);
			}
	...
	public void onSpellManaCost(ManaCostEvent event) {
		if(event.caster instanceof EntityPlayer && AffinityData.For(event.caster)
															   .getAffinityDepth(Affinity.ARCANE) > 0.5f) {
