...
public class EntityAIPlantGuardianThrowSickle extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if(cooldownTicks-- > 0 || host.getCurrentAction() != BossActions.IDLE) {
			return false;
		}
	...
	public boolean continueExecuting() {
		EntityLivingBase AITarget = host.getAttackTarget();
		if(AITarget == null || AITarget.isDead || (host.getCurrentAction() == BossActions.THROWING_SICKLE && host.getTicksInCurrentAction() > host.getCurrentAction()
																																				  .getMaxActionTime()
		)) {
