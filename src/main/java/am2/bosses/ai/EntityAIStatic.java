...
public class EntityAIStatic extends AIAnimation {
	...
	public boolean shouldAnimate() {
		...
		if(living.getAttackTarget() == null || living.getDistanceSqToEntity(living.getAttackTarget()) > 64D || !living.getEntitySenses()
		...
		if(guardian.getAttackTarget() != null) {
			...
			if(guardian.getTicksInCurrentAction() == 20) {
				if(!guardian.worldObj.isRemote) {
					guardian.worldObj.playSoundAtEntity(guardian, "arsmagica2:mob.lightningguardian.static", 1.0f, guardian.getRNG()
																														   .nextFloat() * 0.5f + 0.5f);
				}
			}
			if(guardian.getTicksInCurrentAction() > 66 && guardian.getTicksInCurrentAction() % 15 == 0 && guardian.getEntitySenses()
																												  .canSee(guardian.getAttackTarget())) {
				doStrike();
			}
		}
	}
