...
public class EntityAILightningRod extends AIAnimation {
	...
	public boolean shouldAnimate() {
		...
		if(living.getHealth() > living.getMaxHealth() * 0.75f || living.getAttackTarget() == null || !living.getEntitySenses()
																											.canSee(living.getAttackTarget())) {
			return false;
		}
		...
		if(ticks <= 25) {
			...
			if(ticks > 85 && ticks <= 150) {
				if(!guardian.worldObj.isRemote && ticks % 20 == 0) {
					guardian.worldObj.playSoundAtEntity(guardian, "arsmagica2:mob.lightningguardian.lightning_rod_1", 1.0f, guardian.getRNG()
																																	.nextFloat() * 0.5f + 0.5f);
				}
			}
			...
			if(ticks > 25 && ticks <= 85) {
				...
				if(!guardian.worldObj.isRemote && ticks == 30) {
					guardian.worldObj.playSoundAtEntity(guardian, "arsmagica2:mob.lightningguardian.lightning_rod_start", 1.0f, guardian.getRNG()
																																		.nextFloat() * 0.5f + 0.5f);
				}
			}
				...
				if(!guardian.worldObj.isRemote && ticks % 20 == 0) {
					guardian.worldObj.playSoundAtEntity(guardian, "arsmagica2:mob.lightningguardian.idle", 1.0f, guardian.getRNG()
																														 .nextFloat() * 0.5f + 0.5f);
				}
			}
