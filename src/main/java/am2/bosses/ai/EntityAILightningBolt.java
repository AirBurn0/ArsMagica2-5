		...
		if(guardian.getAttackTarget() != null) {
			...
			if(guardian.getTicksInCurrentAction() == 7) {
				...
				if(!guardian.worldObj.isRemote) {
					guardian.worldObj.playSoundAtEntity(guardian, "arsmagica2:mob.lightningguardian.attack", 1.0f, guardian.getRNG()
																														   .nextFloat() * 0.5f + 0.5f);
				}
			}
		}
	...
	private void doStrike() {
		...
		if(guardian.getAttackTarget() != null && guardian.getEntitySenses().canSee(guardian.getAttackTarget())) {
			...
			if(guardian.getRNG().nextDouble() > 0.2f) {
				...
				AMCore.proxy.particleManager.BoltFromEntityToPoint(guardian.worldObj, guardian, guardian.getAttackTarget().posX - 0.5 + guardian.getRNG()
																																				.nextDouble(), guardian.getAttackTarget().posY - 0.5 + guardian.getRNG()
																																																			   .nextDouble() + guardian.getAttackTarget()
																																																									   .getEyeHeight(), guardian.getAttackTarget().posZ - 0.5 + guardian.getRNG()
																																																																										.nextDouble());
			}
		}
	}
