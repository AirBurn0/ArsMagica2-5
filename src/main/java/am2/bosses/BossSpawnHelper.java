...
public class BossSpawnHelper {
	...
	private void checkForAirGuardianSpawn(World world, int x, int y, int z) {
		...
		if(itemsInRange.get(0).getEntityItem().getItem() != ItemsCommonProxy.essence || itemsInRange.get(0)
																									.getEntityItem()
																									.getItemDamage() != ItemEssence.META_AIR) {
			return;
		}
	...
	private void checkForArcaneGuardianSpawn(World world, int x, int y, int z) {
		...
		if(itemsInRange.get(0).getEntityItem().getItem() != ItemsCommonProxy.essence || itemsInRange.get(0)
																									.getEntityItem()
																									.getItemDamage() != ItemEssence.META_ARCANE) {
			return;
		}
		...
		if(hasStructure && lectern != null && lectern.hasStack() && lectern.getStack()
	...
	private void checkForEarthGuardianSpawn(World world, int x, int y, int z) {
		...
		for(EntityItem item: itemsInRange) {
			...
			else if(item.getEntityItem().getItem() == ItemsCommonProxy.itemOre && item.getEntityItem()
																					  .getItemDamage() == ItemOre.META_BLUETOPAZ) {
				hasTopaz = true;
			}
			else if(item.getEntityItem().getItem() == ItemsCommonProxy.itemOre && item.getEntityItem()
																					  .getItemDamage() == ItemOre.META_CHIMERITE) {
				hasChimerite = true;
			}
		}
	...
	private void checkForFireGuardianSpawn(EntityItem item, World world, int x, int y, int z) {
		if(item.getEntityItem().getItem() != ItemsCommonProxy.essence || item.getEntityItem()
																			 .getItemDamage() != ItemEssence.META_WATER) {
			return;
		}
		boolean hasStructure = false;
	...
	public void checkForEnderGuardianSpawn(World world, int x, int y, int z) {
		...
		for(EntityItem item: itemsInRange) {
			...
			else if(item.getEntityItem().getItem() == ItemsCommonProxy.essence && item.getEntityItem()
																					  .getItemDamage() == ItemEssence.META_ENDER) {
