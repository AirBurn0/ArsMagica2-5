...
public class ParticleManagerClient extends ParticleManagerServer {
						...
						if(effect != null) {
							...
							switch(particleBehaviour) {
								...
								case 3: //orbit
									...
									break;
								case 4: //arc
									effect.AddParticleController(new ParticleArcToEntity(effect, 1, ent, false).generateControlPoints()
	...
	public ParticleController createDefaultParticleController(int type, Object eff, AMVector3 location, float modifier, int meta) {
		...
		switch(type) {
			...
			case 4: //arc
				return new ParticleArcToPoint(effect, 1, location.x, location.y, location.z, false).generateControlPoints()
																								   .SetSpeed(0.02f * modifier);
			case 5: //flee
