...
public class ContainerKeystone extends Container {
	...
	public boolean setInventoryToCombination(int comboIndex) {
		...
		if(player.worldObj.isRemote) {
			AMNetHandler.INSTANCE.sendPacketToServer(AMPacketIDs.SET_KEYSTONE_COMBO, new AMDataWriter().add(comboIndex)
																									   .generate());
			return true;
		}
	...
	private boolean inventoryContainsAllMetas(int[] metas) {
		...
		for(int i: metaQuantities.keySet()) {
			if(!invQuantities.containsKey(i)) {
				return false;
			}
			if(invQuantities.get(i) < metaQuantities.get(i)) {
				return false;
			}
		}
