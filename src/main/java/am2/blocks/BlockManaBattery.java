...
public class BlockManaBattery extends PoweredBlock {
		...
		if(par1World.isRemote) {
			...
			if(te != null) {
				if(AMCore.config.colourblindMode()) {
					par5EntityPlayer.addChatMessage(new ChatComponentText(String.format("Charge Level: %.2f %% [%s]", PowerNodeRegistry.For(par1World)
																																	   .getPower(te, te.getPowerType()) / te.getCapacity() * 100, getColorNameFromPowerType(te.getPowerType()))));
				}
				else {
					par5EntityPlayer.addChatMessage(new ChatComponentText(String.format("Charge Level: %s%.2f \u00A7f%%", te.getPowerType()
																															.chatColor(), PowerNodeRegistry.For(par1World)
																																						   .getPower(te, te.getPowerType()) / te.getCapacity() * 100)));
				}
			}
		}
		...
		if(stack != null) {
			...
			if(stack.stackTagCompound != null) {
				if(stack.stackTagCompound.hasKey("mana_battery_charge") && stack.stackTagCompound.hasKey("mana_battery_powertype")) {
					PowerNodeRegistry.For(par1World)
									 .setPower(te, PowerTypes.getByID(stack.stackTagCompound.getInteger("mana_battery_powertype")), stack.stackTagCompound.getFloat("mana_battery_charge"));
				}
				else {
	...
	private void destroy(World world, int i, int j, int k) {
		...
		if(te != null && !world.isRemote) {
			...
			stack.stackTagCompound.setFloat("mana_battery_charge", PowerNodeRegistry.For(world)
																					.getPower(te, te.getPowerType()));
