	...
	public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace, double impactX, double impactY, double impactZ, EntityLivingBase caster) {
		...
		if(!world.isRemote) {
			if(originalSpellStack.stackTagCompound.hasKey(storageKey)) {
				...
				if(world.isAirBlock(blockx, blocky, blockz) || !world.getBlock(blockx, blocky, blockz)
																	 .getMaterial()
																	 .isSolid()) {
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier) {
		for(int i = 0; i < 5 + 5 * AMCore.config.getGFXLevel(); ++i) {
			...
			if(particle != null) {
				...
				particle.AddParticleController(new ParticleOrbitPoint(particle, x, y, z, 1, false).SetTargetDistance(world.rand.nextDouble() + 0.1f)
																								  .SetOrbitSpeed(0.2f));
			}
		}
	}
