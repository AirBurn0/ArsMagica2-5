		...
		if(!world.isRemote && caster instanceof EntityPlayer && block == BlocksCommonProxy.inertSpawner) {
			...
			if(items != null) {
				...
				if(te != null && te instanceof TileEntityOtherworldAura) {
					((TileEntityOtherworldAura)te).setPlacedByUsername(caster.getCommandSenderName());
				}
				return true;
			}
		}
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier) {
		for(int i = 0; i < 15; ++i) {
			...
			if(particle != null) {
				...
				particle.AddParticleController(new ParticleArcToPoint(particle, 1, target.posX, target.posY + target.getEyeHeight(), target.posZ, false).SetSpeed(0.05f)
		...
		for(int i = 0; i < 15; ++i) {
			...
			if(particle != null) {
				particle.addRandomOffset(1, 1, 1);
				particle.AddParticleController(new ParticleArcToPoint(particle, 1, caster.posX, caster.posY + caster.getEyeHeight(), caster.posZ, false).SetSpeed(0.05f)
																																						.generateControlPoints());
				particle.setMaxAge(40);
				particle.setParticleScale(0.2f);
