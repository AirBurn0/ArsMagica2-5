	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier) {
		for(int i = 0; i < 25; ++i) {
			...
			if(particle != null) {
				...
				particle.AddParticleController(new ParticleOrbitPoint(particle, x, y, z, 2, false).SetOrbitSpeed(0.5f)
																								  .setIgnoreYCoordinate(true)
																								  .SetTargetDistance(0.3f + rand.nextDouble() * 0.3));
				particle.AddParticleController(new ParticleFadeOut(particle, 3, false).setFadeSpeed(0.05f)
