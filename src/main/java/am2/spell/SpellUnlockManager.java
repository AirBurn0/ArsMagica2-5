		...
		if(event.caster instanceof EntityPlayer) {
			...
			for(UnlockEntry entry: entries) {
				...
				if(!(entry.unlock instanceof ISpellModifier)) {
					...
					if(entry.unlockIsInSpell(event.stack) && (!SkillData.For((EntityPlayer)event.caster)
																		.isEntryKnown(SkillTreeManager.instance.getSkillTreeEntry(entry.unlock)) || !entry.unlockIsInPrimaryTree((EntityPlayer)event.caster)
					)) {
