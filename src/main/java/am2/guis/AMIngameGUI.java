...
public class AMIngameGUI {
	...
	private void RenderSpellBookUI(int i, int j, FontRenderer fontrenderer, ItemStack bookStack) {
		...
		for(int n = 0; n < 8; ++n) {
			...
			int d = 12;
			DrawIconAtXY(stackItem.getItem().getIconFromDamageForRenderPass(stackItem.getItemDamage(), 0), "items", IIconX, spellUI_y + 1.5f, d, d, false);
			DrawIconAtXY(stackItem.getItem().getIconFromDamageForRenderPass(stackItem.getItemDamage(), 1), "items", IIconX, spellUI_y + 1.5f, d, d, false);
		}
	...
	private void RenderManaBar(int i, int j, FontRenderer fontRenderer) {
		...
		if(AMCore.config.showHudBars()) {
			...
			String magicLevel = String.valueOf(ExtendedProperties.For(mc.thePlayer).getMagicLevel());
	...
	public void RenderAffinity(int i, int j) {
		...
		for(Affinity affinity: ad.getHighestAffinities()) {
			...
			GL11.glColor3f(1.0f, 1.0f, 1.0f);
			AMGuiHelper.DrawIconAtXY(affinity.representItem.getIconFromDamage(affinity.representMeta), x, y, j, 12, 12, true);

			if(AMCore.config.getShowNumerics()) {
				String display = String.format("%.2f%%", AffinityData.For(mc.thePlayer)
																	 .getAffinityDepth(affinity) * 100);
				if(x < i / 2) {
		...
		for(PotionEffect pe: getPotionEffectsByTimeRemaining()) {
			...
			if(potion.isBadEffect()) {
				if(potion.hasStatusIcon()) {
					...
					this.drawTexturedModalRect_Classic(negative_buff_x, negative_buff_y, l % 8 * 18, 198 + l / 8 * 18, 10, 10, 18, 18);
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
					negative_buff_x -= 12;
				...
				if(potion.hasStatusIcon()) {
					...
					this.drawTexturedModalRect_Classic(positive_buff_x, positive_buff_y, l % 8 * 18, 198 + l / 8 * 18, 10, 10, 18, 18);
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
					positive_buff_x += 12;
	...
	public void drawTexturedModalRect_Classic(int par1, int par2, int par3, int par4, int par5, int par6) {
		...
		var9.addVertexWithUV(par1, par2 + par6, this.zLevel, (par3) * var7, (par4 + par6) * var8);
		var9.addVertexWithUV(par1 + par5, par2 + par6, this.zLevel, (par3 + par5) * var7, (par4 + par6) * var8);
		var9.addVertexWithUV(par1 + par5, par2, this.zLevel, (par3 + par5) * var7, (par4) * var8);
		var9.addVertexWithUV(par1, par2, this.zLevel, (par3) * var7, (par4) * var8);
		var9.draw();
	}
	...
	public void drawTexturedModalRect(int par1, int par2, int par3, int par4, int par5, int par6) {
		...
		var8.addVertexWithUV(par1, par2 + par6, this.zLevel, var9, var12);
		var8.addVertexWithUV(par1 + par5, par2 + par6, this.zLevel, var10, var12);
		var8.addVertexWithUV(par1 + par5, par2, this.zLevel, var10, var11);
		var8.addVertexWithUV(par1, par2, this.zLevel, var9, var11);
		var8.draw();
	}
