...
public class GuiEssenceRefiner extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if(essenceExtractorInventory.isRefining() && PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
																	  .getHighestPower(essenceExtractorInventory) < TileEntityEssenceRefiner.TICK_REFINE_COST && AMGuiHelper.instance.getFastTicker() % 20 < 10) {
	...
	public void drawTexturedModalRect_Classic(int dst_x, int dst_y, int src_x, int src_y, int dst_width, int dst_height, int src_width, int src_height) {
		...
		var9.addVertexWithUV(dst_x, dst_y + dst_height, this.zLevel, 0, 1);
		var9.addVertexWithUV(dst_x + dst_width, dst_y + dst_height, this.zLevel, 1, 1);
		var9.addVertexWithUV(dst_x + dst_width, dst_y, this.zLevel, 1, 0);
		var9.addVertexWithUV(dst_x, dst_y, this.zLevel, 0, 0);
		var9.draw();
	}
