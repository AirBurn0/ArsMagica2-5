...
public class LightningBoltCommon {
	...
	public LightningBoltCommon(World world, AMVector3 jammervec, AMVector3 targetvec, long seed) {
		...
		this.boundingBox.setBB(AxisAlignedBB.getBoundingBox(Math.min(this.start.x, this.end.x), Math.min(this.start.y, this.end.y), Math.min(this.start.z, this.end.z), Math.max(this.start.x, this.end.x), Math.max(this.start.y, this.end.y), Math.max(this.start.z, this.end.z))
											.expand(this.length / 2.0F, this.length / 2.0F, this.length / 2.0F));
	...
	public void fractal(int splits, float amount, float splitchance, float splitlength, float splitangle) {
		...
		for(Iterator iterator = oldsegments.iterator(); iterator.hasNext(); ) {
			...
			for(int i = 1; i < splits; i++) {
				AMVector3 randoff = AMVector3.getPerpendicular(segment.diff)
			...
			for(int i = 0; i < splits; i++) {
				...
				if((i != 0) && (this.rand.nextFloat() < splitchance)) {
					AMVector3 splitrot = AMVector3.xCrossProduct(next.diff)
												  .rotate(this.rand.nextFloat() * 360.0F, next.diff);
					AMVector3 diff = next.diff.copy()
					...
					Segment split = new Segment(newpoints[i], new BoltPoint(newpoints[(i + 1)].basepoint, newpoints[(i + 1)].offsetvec.copy()
																																	  .add(diff)), segment.light / 2.0F, next.segmentno, this.numsplits);
					split.prev = prev;
