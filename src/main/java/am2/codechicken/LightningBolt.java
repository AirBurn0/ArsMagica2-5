...
public class LightningBolt extends EntityFX {
	...
	private final LightningBoltCommon main;
	...
	private void renderBolt(Tessellator tessellator, float partialframe, float cosyaw, float cospitch, float sinyaw, float cossinpitch, int pass) {
		...
		for(Iterator iterator = this.main.segments.iterator(); iterator.hasNext(); ) {
			...
			if(rendersegment.segmentno <= renderlength) {
				...
				AMVector3 diff1 = AMVector3.crossProduct(playervec, rendersegment.prevdiff)
										   .scale(width / rendersegment.sinprev);
				AMVector3 diff2 = AMVector3.crossProduct(playervec, rendersegment.nextdiff)
				...
				if(rendersegment.next == null) {
					AMVector3 roundend = rendersegment.endpoint.point.copy()
				...
				if(rendersegment.prev == null) {
					AMVector3 roundend = rendersegment.startpoint.point.copy()
